...
require "kintsugi/apply_changes_to_project"
require "kintsugi/remove_redundant_new_target_settings"

require_relative "be_equivalent_to_project"

describe Kintsugi, :apply_changes_to_project do
  ...
  it "adds new target" do
    ...
    described_class.remove_redundant_new_target_settings(target)

    changes_to_apply = get_diff(theirs_project, base_project)

    described_class.apply_changes_to_project(base_project, changes_to_apply)
    base_project.save
  ...
  it "adds new subproject" do
    ...
    expect(base_project).to be_equivalent_to_project(theirs_project, ["containerPortal"])
  end
  ...
  describe "file related changes" do
    ...
    it "adds file inside a group that has a path on filesystem" do
      theirs_project = create_copy_of_project(base_project.path, "thiers")
    ...
    describe "dealing with unexpected change" do
      it "ignores change to a file whose containing group doesn't exist" do
        ...
        described_class.apply_changes_to_project(ours_project, changes_to_apply)
        ours_project.save
  ...
  describe "target related changes" do
    ...
    it "changes framework from to reference proxy file reference" do
      framework_filename = "baz"
    ...
    # This test checks that Kintsugi detects incorrect change output of project diff.
    it "raises when adds subproject target and adds reference to it" do
      framework_filename = "baz"
    ...
    it "adds target dependency" do
      second_target = base_project.new_target("com.apple.product-type.library.static", "bar", :ios)
      described_class.remove_redundant_new_target_settings(second_target)
    ...
    it "adds base configuration reference to new target" do
      configuration_reference = base_project.main_group.new_reference("baz")
  ...
  def add_new_subproject_to_project(project, subproject_name, subproject_product_name)
    ...
    target = subproject.new_target("com.apple.product-type.library.static", subproject_product_name,
                                   :ios)
    described_class.remove_redundant_new_target_settings(target)
    subproject.save

    subproject_reference = project.new_file(subproject_path, :built_products)
